...
public interface SalesActivityRepository extends JpaRepository<ParkingSaleActivity, String> {
	...
	@Query("select u from ParkingSaleActivity u where u.createdDate > ?1 and u.createdDate < ?2 and u.activityHolder = ?3")
	List<ParkingSaleActivity> getParkingSaleActivityBetweenForUser(DateTime startTime, DateTime endTime,
			User activityHolder);
	...
	@Query("select u from ParkingSaleActivity u where u.parkingStatus = ?1 and u.createdDate > ?2")
	List<ParkingSaleActivity> getParkingSaleActivitiesByParkingStatus(String parkingStatus, DateTime startTime);
